...
bar = Bar(rectangle, bar_color=(255, 125, 0), bar_inverse_color=(255, 0, 0), bar_closed=True,
          start_fill_side=Placement.BOTTOM, text=Text('Test', font_size=30, alignment=Placement.TOP_OUT),
          fit_text=False)

moving_block = Rect(50, 500, 50, 50, color=(255, 0, 0, 0), border=20)
moving_block_sqr = Rect(48, 498, 94, 94, color=(0, 0, 0), border=2)
move_amount = 40
moving_block_seq_pos = 0

moving_block_right = oa([oa.Action(oa.Action.MOVE, {'x': move_amount, 'time': 20})],
                        animation_objects=(moving_block,))
moving_block_left = oa([oa.Action(oa.Action.MOVE, {'x': -move_amount, 'time': 20})],
                       animation_objects=(moving_block,))
moving_block_up = oa([oa.Action(oa.Action.MOVE, {'y': -move_amount, 'time': 20})],
                     animation_objects=(moving_block,))
moving_block_down = oa([oa.Action(oa.Action.MOVE, {'y': move_amount, 'time': 20})],
                       animation_objects=(moving_block,))

blue_block_color = oa([oa.Action(oa.Action.SET_COLOR_TO, {'color': (150, 150, 255)})],
                      animation_objects=(moving_block,))
red_block_color = oa([oa.Action(oa.Action.SET_COLOR_TO, {'color': (255, 0, 0)})],
                     animation_objects=(moving_block,))

block_border = oa([oa.Action(oa.Action.CHANGE_BORDER_WIDTH_TO, {'border': 20, 'time': 10})],
                  animation_objects=(moving_block,))
no_block_border = oa([oa.Action(oa.Action.CHANGE_BORDER_WIDTH_TO, {'border': 5, 'time': 10})],
                     animation_objects=(moving_block,))

scale_up_block = oa([oa.Action(oa.Action.SCALE, {'width': move_amount, 'time': 10})],
                    animation_objects=(moving_block,))
scale_back_block = oa([oa.Action(oa.Action.SCALE, {'width': -move_amount, 'time': 10})],
                      animation_objects=(moving_block,))

command_field = InputField(Rect(50, 700, 500, 60, color=(200, 200, 200), corner_radius_all=20, border=5),
                           empty_text=Text('command here', color=(150, 150, 150)), rect_active_color=(0, 150, 0))
...
move_block_button = Button(Rect(150, 500, 100, 50, color=(0, 0, 0)),
                           text=Text('Move Block', margin=3, color=(255, 0, 0)),
...
print_words_button = Button(Rect(100, 550, 100, 100, color=(200, 200, 200)), text=Text('Print Words', margin=3),
                ...
                case pygame.KEYDOWN:
                    ...
                    match event.key:
                        ...
                        case pygame.K_3:
                            ...
                            print(f'{bar._display_range} -> {bar._goal_value_range} / {bar.max_value_range}')
